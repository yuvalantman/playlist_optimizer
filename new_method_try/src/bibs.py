...
class BIBSConfig:
    ...
    # Stochastic random-walk search (Chapter 2 intent). When False the algorithm is
    # fully deterministic and reproduces the original behavior exactly.
    stochastic: bool = False
    top_k: int = 5
    temperature: float = 0.15
    random_seed: int = 42
    # Commit the chosen forward/backward beam paths (meet-in-the-middle) instead of
    # discarding them and re-solving by recursion. When the beams fully cover the
    # interval, the interval is solved in a single level.
    commit_beams: bool = False
    # Bottleneck usage. "score" (default) subtracts a bottleneck bonus from placement
    # scores (original behavior). "eligibility" removes that direct discount so arc and
...
class BIBS:
    ...
    def _validate_config(self) -> None:
        ...
        if self.config.bottleneck_mode not in ("score", "eligibility"):
    ...
    def _bottleneck_bonus(
        ...
        In "eligibility" mode the direct score discount is removed (returns 0): the
        bottleneck signal then only shapes candidate exposure/ordering via the
        orchestrator, not the transition-sensitive placement score.
        """
        if self.config.bottleneck_mode == "eligibility":
            return 0.0
    ...
    def _expand_beam(
        ...
        for chunk_index, position_index_in_chunk, position in chunked_positions():
            ...
            if self.config.stochastic and len(expanded) > self.config.beam_width:
                chosen_indices = softmax_topk_choice(
                    np.asarray([beam.score for beam in expanded], dtype=float),
                    self.config.beam_width,
                    self._rng,
                    self.config.top_k,
                    self.config.temperature,
                )
                beams = [expanded[index] for index in chosen_indices]
            else:
    ...
    def _select_anchor(
        ...
        target_arc: np.ndarray,
        track_pool: pd.DataFrame,
    ) -> tuple[int, int, int, float, dict, "_BeamItem | None", "_BeamItem | None"]:
        del target_arc, track_pool
        self._diagnostics["candidate_orchestrator_calls"] += 1
        ...
        best_neighbors_by_candidate: dict[int, tuple[int, int]] = {}
        best_forward_by_candidate: dict[int, _BeamItem | None] = {}
        best_backward_by_candidate: dict[int, _BeamItem | None] = {}
            ...
            for backward in backward_beams:
                ...
                for anchor in anchor_candidates:
                    ...
                    if candidate_score < best_scores_by_candidate.get(
                        ...
                        best_neighbors_by_candidate[anchor] = (
                            left_neighbor,
                            right_neighbor,
                        )
                        best_forward_by_candidate[anchor] = forward
                        best_backward_by_candidate[anchor] = backward
                    key = (
                        candidate_score,
                        anchor,
                        left_neighbor,
        ...
        if best is None:
            ...
            best_components_by_candidate[anchor] = components
            best_neighbors_by_candidate[anchor] = (left_track, right_track)
            best_forward_by_candidate[anchor] = None
            best_backward_by_candidate[anchor] = None
        ...
        # Stochastic random-walk anchor selection: sample among the best-scoring
        # candidates instead of taking the deterministic minimum. Deterministic mode
        # (stochastic=False) leaves the original `best` selection untouched.
        if self.config.stochastic and ranked_candidates:
            sampled = softmax_topk_choice(
                np.asarray(
                    [best_scores_by_candidate[c] for c in ranked_candidates],
                    dtype=float,
                ),
                1,
                self._rng,
            ...
            if sampled:
                anchor = ranked_candidates[sampled[0]]
                score = best_scores_by_candidate[anchor]
                components = best_components_by_candidate[anchor]
                left_neighbor, right_neighbor = best_neighbors_by_candidate[anchor]
        chosen_forward = best_forward_by_candidate.get(anchor)
        chosen_backward = best_backward_by_candidate.get(anchor)
    ...
    def _solve_interval(
        ...
        # Commit mode: write the chosen forward/backward beam paths directly when they
        # fully cover their positions, solving the whole interval in one level (a
        # meet-in-the-middle commit) instead of discarding the beams and recursing.
        if (
            self.config.commit_beams
            and chosen_forward is not None
            and chosen_backward is not None
            and len(chosen_forward.path) == len(forward_positions)
            and len(chosen_backward.path) == len(backward_positions)
        ):
            for position, track in zip(forward_positions, chosen_forward.path):
                playlist[position] = int(track)
                available_tracks.remove(int(track))
            for position, track in zip(backward_positions, chosen_backward.path):
                playlist[position] = int(track)
                available_tracks.remove(int(track))
            return
